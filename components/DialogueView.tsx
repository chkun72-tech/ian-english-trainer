      ...
      {dialogue.lines.map((line, i) => {
        ...
        return (
          <div key={i} className={`flex ${isYou ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${isYou ? "bg-accent text-white" : "bg-white border border-line"}`}>
              ...
              {isYou && isOpen && (
                <p className="text-[11px] text-white/70 mt-1">{line.zhHint}</p>
              )}
            </div>
          </div>
        );
